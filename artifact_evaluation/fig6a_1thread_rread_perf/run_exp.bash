...
do
    ...
    sudo $FIO_DIR/fio ${FIO_OPTIONS} ${WORKLOAD_FILE} 2>&1 | tee $SCRIPT_DIR/results/baseline_${BLK_SIZE}KB.out
done
...
do
    sed -i 's/bs=.*K/bs='${BLK_SIZE}'K/g' ${WORKLOAD_FILE}
    sudo $FIO_DIR/fio ${FIO_OPTIONS} ${WORKLOAD_FILE} 2>&1 | tee $SCRIPT_DIR/results/libaio_${BLK_SIZE}KB.out
done
...
do
    sed -i 's/bs=.*K/bs='${BLK_SIZE}'K/g' ${WORKLOAD_FILE}
    sudo $FIO_DIR/fio ${FIO_OPTIONS}  ${WORKLOAD_FILE} 2>&1 | tee $SCRIPT_DIR/results/iouring_${BLK_SIZE}KB.out
done
...
do
    sed -i 's/bs=.*K/bs='${BLK_SIZE}'K/g' ${WORKLOAD_FILE}
    sudo LD_PRELOAD=${USERLIB_DIR}/libshim.so $FIO_DIR/fio ${FIO_OPTIONS} ${WORKLOAD_FILE} 2>&1 | tee $SCRIPT_DIR/results/bypassd_${BLK_SIZE}KB.out
done
