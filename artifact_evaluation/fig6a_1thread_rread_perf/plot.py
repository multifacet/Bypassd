...
plt.ylabel("Read latency (us)", size=18)
plt.yticks([10,20,30,40])
plt.xlabel("Read bandwidth (GB/s)", size=18)
...
for x,y in zip(sync_bw,sync_lat):
    ...
    i += 1
    if i == 6:
        plt.annotate(label,(x,y),textcoords="offset points",xytext=(-25,-10), ha='center',fontsize=12)
    elif i == 5:
        plt.annotate(label,(x,y),textcoords="offset points",xytext=(-5,10), ha='center',fontsize=12)
    else:
